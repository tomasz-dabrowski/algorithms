...
% Wykresy kolumn D, E i F w zale¿noœci od kolumny A (01.png)
...
% Wykres schodkowy kolumn D, F w zale¿noœci od kolumny A (02.png)
...
% Wykres s³upkowy kolumn D, E, F w zale¿noœci od kolumny A (03.png)
...
% Generujemy nastêpuj¹ce wykresy na jednym rysunku (04.png)

figure

% 1. Wykres w formie próbek kolumn D, F w zale¿noœci od kolumny A

subplot(2,2,1) % pierwsza æwiartka
stem(a,d,'b') % wykres D od A
hold on % na³ó¿ drugi wykres
stem(a,f,'r') % F od A
title('Wykres próbkowy')
xlabel('Oœ X')
ylabel('Oœ Y')
grid
axis([-1 32 -260 170])

% 2. Wykres s³upkowy kolumn D, E, F w zale¿noœci od kolumny A 

subplot(2,2,2) % druga æwiartka
...
% lub tak:
% b = bar(a,x); % x macierz [a d e f] przygotoana wczeœniej
% set(b(1), 'FaceColor', 'g')
% set(b(2), 'FaceColor', 'y')
% set(b(3), 'FaceColor', 'r')
...
grid
ylim([-300 850]) % przesuniêcie wzglêdem osi y

% 3. Wykres liniowy (znaczniki nie po³¹czone lini¹) kolumny D od kolumny A

subplot(2,2,3) % trzecia æwiartka
plot(a,d,'bo','markerfacecolor','y')
title('Wykres liniowy znaczników')
xlabel('Oœ X')
ylabel('Oœ Y')
grid
axis([0 31 -250 50])

% Wykres liniowy kolumny F od A z pogrubion¹ lini¹, bez znaczników (04.png)

subplot(2,2,4) % czwarta æwiartka
